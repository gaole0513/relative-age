...
dx_rx_adhd <- rbind(dx_adhd,rx_adhd) %>% 
...
dat0 <- dat %>% 
  mutate(y2001=ifelse(birth_yr==2001,1,0),
         ...
         ses_m1=ifelse(ses=="Midium1",1,0),
         ses_m2=ifelse(ses=="Midium2",1,0),
...
varsp <- c("event","Sex.","y2001","y2002","y2003","y2004","y2005","y2006","y2007","y2008","y2009","y2010","lb","preterm","Baby.s.Info..With.Birth.Trauma.mentioned..Y.N..","apgar0","apgar1","apgar2","apgar3","ses_l","ses_m1","ses_m2","ses_u","mode_n","mode_v","mode_b","mode_c","mode_f","mode_o")
catvarsp<- c("event","Sex.","y2001","y2002","y2003","y2004","y2005","y2006","y2007","y2008","y2009","y2010","lb","preterm","Baby.s.Info..With.Birth.Trauma.mentioned..Y.N..","apgar0","apgar1","apgar2","apgar3","ses_l","ses_m1","ses_m2","ses_u","mode_n","mode_v","mode_b","mode_c","mode_f","mode_o")
basTable<-CreateTableOne( data = dat0,strata = "exp",vars = varsp, factorVars = catvarsp, addOverall=T)
baseline<-as.data.frame(as.data.table(print(basTable,smd = T,test = T,noSpaces = T,showAllLevels=F),keep.rownames = T))


options(digits = 10)
#logistic
#crude
result <- glm(event ~ exp, family = "binomial", data = dat)
exp(coef(result))
exp(confint(result))

#adjusted
result <- glm(event ~ exp+Sex.+as.factor(birth_yr)+lb+preterm+Baby.s.Info..With.Birth.Trauma.mentioned..Y.N..+as.factor(apgar)+as.factor(ses)+as.factor(d_mode), family = "binomial", data = dat)
exp(coef(result))
exp(confint(result))

c <- clogit(event ~ exp+Sex.+as.factor(birth_yr)+lb+preterm+Baby.s.Info..With.Birth.Trauma.mentioned..Y.N..+as.factor(apgar)+as.factor(ses)+as.factor(d_mode), data = dat)
summary(c)

c <- coxph(Surv(rep(1, 77986L), event) ~ exp+Sex.+strata(birth_yr)+lb+preterm+Baby.s.Info..With.Birth.Trauma.mentioned..Y.N..+as.factor(apgar), 
...
# result.cox1 <- coxph(Surv(fu_days, event) ~ exp+Sex.+strata(birth_yr)+lb+preterm+Baby.s.Info..With.Birth.Trauma.mentioned..Y.N..+as.factor(apgar), 
#                      data =dat,x=T)
...
#                data =dat)
#                weights = w,)
weigthts_weight <- weightit(exp ~ Sex.+as.factor(birth_yr)+lb+preterm+Baby.s.Info..With.Birth.Trauma.mentioned..Y.N..+as.factor(apgar)+as.factor(ses)+as.factor(d_mode),
                           data = dat,
...
library(devEMF)
png("aim1.png",width = 9, height = 6, units = 'in',res = 600)
fm1
dev.off()
...
summary(result.cox1)
#adjusted
result.cox1 <- coxph(Surv(fu_days, event) ~ exp+Sex.+strata(birth_yr)+lb+preterm+Baby.s.Info..With.Birth.Trauma.mentioned..Y.N..+as.factor(apgar)+as.factor(ses)+as.factor(d_mode), 
                     data =dat)
...
#poisson regression
#crude
exp(summary(glm(event ~ exp,offset = log(fu_days),family="poisson",data=dat))$coefficients[2,1])
#confindence interval
exp(confint(glm(event ~ exp,offset = log(fu_days),family="poisson",data=dat)))
#adjusted
exp(summary(glm(event ~ exp+Sex.+as.factor(birth_yr)+lb+preterm+Baby.s.Info..With.Birth.Trauma.mentioned..Y.N..+as.factor(apgar)+as.factor(ses)+as.factor(d_mode),offset = log(fu_days),family="poisson",data=dat))$coefficients[2,1])
#confindence interval
exp(confint(glm(event ~ exp+Sex.+as.factor(birth_yr)+lb+preterm+Baby.s.Info..With.Birth.Trauma.mentioned..Y.N..+as.factor(apgar)+as.factor(ses)+as.factor(d_mode),offset = log(fu_days),family="poisson",data=dat)))
...
py <- dat %>% 
  ...
  mutate(inci=event_sum/indiv_sum*100,
         ir=event_sum/py_sum*1000)
table(dat$exp,dat$event)
# summary(coxph(Surv(fu_days, event) ~ exp+Sex.+as.factor(birth_yr), data =  dat))
# ggsurvplot(
...
# ggsave("D:/OneDrive - The University Of Hong Kong/other share/5.PhD project/38.relative age/1. ADHD persistence/km_aim1.emf",plot = fm,width = 9, height = 6)
library(devEMF)
png("aim2.png",width = 9, height = 6, units = 'in',res = 600)
fm1
dev.off()
...
exp(summary(glm(event ~ exp+Sex.+as.factor(birth_yr)+lb+preterm+Baby.s.Info..With.Birth.Trauma.mentioned..Y.N..+as.factor(apgar)+as.factor(ses)+as.factor(d_mode),offset = log(fu_days),family="poisson",data=dat))$coefficients[2,1])
#confindence interval
exp(confint.default(glm(event ~ exp+Sex.+as.factor(birth_yr)+lb+preterm+Baby.s.Info..With.Birth.Trauma.mentioned..Y.N..+as.factor(apgar)+as.factor(ses)+as.factor(d_mode),offset = log(fu_days),family="poisson",data=dat)))
...
# ggsave("D:/OneDrive - The University Of Hong Kong/other share/5.PhD project/38.relative age/1. ADHD persistence/km_aim1.emf",plot = fm,width = 9, height = 6)
library(devEMF)
png("aim3.png",width = 9, height = 6, units = 'in',res = 600)
fm1
dev.off()
