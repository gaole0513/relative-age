...
bb0117 <- merge(mc_01to23 %>% select(-Date.of.Birth..yyyy.mm.dd..,-Date.of.Registered.Death.),
  ...
  filter(is.na(Date.of.Registered.Death.)|Date.of.Registered.Death.>age6) %>% 
...
bb0117<- merge(bb0117,sga,by="Maternity.Episode..Gestation.weeks.1") %>% 
  mutate(lb=ifelse(as.numeric(Baby.s.Info..Birth.Weight..gm..)<sga,1,0))%>% #658128
  filter(year(DOB)>=2001,
         year(DOB)<=2006)
...
final2 <- merge(final1 %>% select(-adhd_date),
  ...
  mutate(edate=pmin(age18-1,
                    ...
                    ymd('2019-12-31'),
                    na.rm = TRUE)) %>% 
  mutate(event=if_else(adhd_date>=age13&adhd_date<=edate,1,0)) %>%
  mutate(event=if_else(is.na(event),0,event)) %>% 
...
# fm_data <- readRDS("M:/Personal/Gao Le/to_fm/sen_lim_aim3.rds")
# summary(result.cox_fm)

# gl_data <- dat
# aaa <- merge(fm_data %>% select(sen_time,sen_out,reference_key,expo,sex,birth_year,low_birth_weight,preterm,babys_info_with_birth_trauma_mentioned_y_n,apgar_score_at_5_min),
#              gl_data %>% select(fu_days,event,Baby.s.Reference.Key.,exp,Sex.,birth_yr,lb,preterm,Baby.s.Info..With.Birth.Trauma.mentioned..Y.N..,apgar),by.x = "reference_key",by.y = "Baby.s.Reference.Key.")
# aaa[!sen_time==fu_days]
